...
class SearchState:
    ...
    def __init__(self, position, unvisited_corners):
        self.position = tuple(position)
        self.unvisited = set(unvisited_corners)        
        if position in self.unvisited:
            self.unvisited.remove(position)
    
    def generateSuccessor(self,x, y):
        return SearchState((x, y), self.unvisited)
    ...
    def __str__(self):
        return '%s, %s' % (self.position, list(sorted(self.unvisited)))
...
def foodHeuristic(state, problem):
    ...
    if len(food_list) == 0:
        ...
        return abs_distance(food_list[0], position)

    from itertools import combinations
    all_pairs = combinations(set(foodGrid.asList()), 2)
    all_distances = [(abs_distance(*pair), pair) for pair in all_pairs]
    max_distance, closet_to_me = 0, 0
    if len(all_distances):
      max_distance, max_pair = max(all_distances, key=lambda p: p[0])
      closet_to_me = min(abs_distance(max_pair[0], position),abs_distance(max_pair[1], position))

    return max_distance + closet_to_me
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        queue = util.Queue()
        explored_nodes = []
        current_node = search.SearchNode(problem.getStartState())
        while not problem.isGoalState(current_node.state):
            ...
            for next_state, next_act, next_cost in problem.getSuccessors(current_node.state):
                next_node = search.SearchNode(next_state,next_act, next_cost, current_node)
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        x,y = state
        return self.food[x][y] # If there is a food on this cell, return True. Otherwise false
